...
namespace ReleaseTool
{
    ...
    internal class ReleaseCommand
    {
        ...
        public class Options: GitHubClient.IGitHubOptions
        {
            ...
            [Option('u', "pull-request-url", HelpText = "The link to the release candidate branch to merge.", Required = true)]
        ...
        public int Run()
        {
            try
            {
                ...
                {
                    throw new InvalidOperationException($"Was unable to merge pull request at: {options.PullRequestUrl}. Received error: {mergeResult.Message}");
                }
        ...
        private static (string, int) ExtractPullRequestInfo(string pullRequestUrl)
        {
            ...
            {
                throw new Exception($"Parsing pull request URL failed. Expected number for pull request id, received: {pullRequestIdStr}");
            }
